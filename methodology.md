...
The baseline system extracts 40 MFCC coefficients per frame and aggregates them over time by taking the temporal mean. If $MFCC_t \in \mathbb{R}^{40}$ denotes the MFCC vector at frame $t$ and $T$ is the number of frames in the utterance, the global MFCC mean is computed as:
...
To capture multi-scale temporal trends, the envelope is smoothed using sliding rectangular windows of different lengths (Short: 50ms, Long: 200ms):
...
Higher-order moments of the envelope distribution (Skewness and Kurtosis) are also computed:
...
The number of loudness spikes exceeding a three standard deviation threshold is defined as:

$$
N_{\text{spike}} = \sum_{m} f(\Delta L[m])
$$

where $f(x) = x$ if $x > 3\sigma_{\Delta L}$, and 0 otherwise.

---

### C. Enhanced Modulation Spectrum Features

Modulation domain features are derived from windowed frames of the amplitude envelope. The modulation power spectrum for the $i$-th frame is given by:

$$
M_i(\omega) = \big| \text{FFT}\big(f_i[n]\big) \big|^{2}
$$

Modulation power is summarized over three modulation frequency bands (0-20 Hz, 20-50 Hz, and 50-100 Hz) using statistics such as mean, standard deviation, and range.
...
To characterize temporal stability of the background, a background-only temporal energy trajectory $B_{\text{temp}}[m]$ is defined. Sudden background changes are counted whenever the frame-to-frame change exceeds a fixed threshold ($2\sigma$):
...
All features are concatenated into a single utterance-level feature vector:
...
Where the baseline system uses only the MFCC mean vector, and the enhanced system makes use of the complete set.
